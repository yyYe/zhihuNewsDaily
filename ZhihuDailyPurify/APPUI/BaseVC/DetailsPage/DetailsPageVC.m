...
- (void)viewDidLoad {
    ...
    [testButton setImage:[UIImage imageNamed:@"xiazai"] forState:UIControlStateNormal];
...
- (void)testButtonTapped {
    ShowInfoVC *showInfo = [ShowInfoVC new];
    [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
    showInfo.Title = [self.webView stringByEvaluatingJavaScriptFromString:@"document.title"];
    [self.navigationController addChildViewController:showInfo];
    [self.navigationController.view addSubview:showInfo.view];
    NSLog(@"title-%@",showInfo.Title);
}
...
- (void)rightBtn5Tapped:(UIButton *)sender {
    if (sender.tag == kButtonStatusStart) {
        btnImage(sender, @"yellowStar40Enabled");
        sender.tag = kButtonStatusStop;
    }else {
        btnImage(sender, @"yellowStar40Disabled");
        sender.tag = kButtonStatusStart;
    }
}
